...
CR.Main = {
    ...
    cloneItem: function(product,i){
        ...
        if ( product.image == null ) {
            item_product.find('img').attr('src','//theme.hstatic.net/200000337429/1000710513/14/no_image.jpg?v=399').attr('alt', product.url);
        } else {
        ...
        jQuery.getJSON('/cart.js', function(cart, textStatus) {
            if(cart) {
                ...
                if($('.sticky-right').hasClass('active')){
                    $('.cart-mobile .flex-box').html('<span>'+cart.items.length+' MÃ³n</span><span>'+Haravan.formatMoney(cart.total_price, formatMoney)+'</span><span>ÄÃ³ng</span>');
                }
                else{
                    $('.cart-mobile .flex-box').html('<span>'+cart.items.length+' MÃ³n</span><span>'+Haravan.formatMoney(cart.total_price, formatMoney)+'</span><span>Xem chi tiáº¿t</span>');
                }
                $('.count-holder').html(cart.item_count);
                $('.cart-mobile .flex-box').html('<span>'+cart.items.length+' MÃ³n</span><span>'+Haravan.formatMoney(cart.total_price, "")+'</span><span>ÄÃ³ng</span>');
                if(cart.item_count == 0){
    ...
    orderFunction: function(){
        function check_time(){
            ...
            if(hours == hstart){
                if(minute >= mstart ){
                    $('.btn-check a').addClass('over-time').html('ÄÃ£ ngÆ°ng nháº­n Ä‘Æ¡n hÃ´m nay');
                    $('.action-cart a').addClass('over-time');
                }
                else{
                    $('.btn-check a').removeClass('over-time').html('Thanh toÃ¡n');
        ...
        one = new Date(one);
        two = new Date(two);
        choise_day += '<option data-check="oke" value="'+ format_date(fix_d.getDate()) +'-'+ format_date(fix_d.getMonth() + 1) +'-'+ fix_d.getFullYear() +'" >HÃ´m nay</option>';
        choise_day += '<option data-check="" value="'+ format_date(one.getDate()) +'-'+ format_date(two.getMonth() + 1) +'-'+ fix_d.getFullYear() +'" >NgÃ y '+ format_date(one.getDate()) +'-'+ format_date(one.getMonth() + 1) +'</option>';
        choise_day += '<option data-check="" value="'+ format_date(one.getDate()) +'-'+ format_date(two.getMonth() + 1) +'-'+ fix_d.getFullYear() +'" >NgÃ y '+ format_date(two.getDate()) +'-'+ format_date(two.getMonth() + 1) +'</option>';
        ...
        $('.sub-choise-now').click(function(){
            var _this = $(this);
            var time = 'Giao ngay';
            var timeText = '<span class="timebox"><span class="textday">Giao ngay</span></span><span class="btnchange">Thay Ä‘á»•i</span>';
        ...
        $('.btn-choise-submit').click(function(e){
            ...
            var timeText = '<span class="timebox">Giao ngÃ y: <span class="textday">' + $('.choise-day').val() + '</span> - LÃºc: <span class="texttime">' + $('.choise-time').val() + '</span></span><span class="btnchange">Thay Ä‘á»•i</span>';
            $('.btn-choise-time span').html(timeText);
            var _this = $(this);
            $.ajax({
                type: "POST",
        ...
        $('.cart-mobile').click(function(){
            if($(this).parent('.sticky-right').hasClass('active')){
                ...
                $(this).find('.flex-box span').eq(2).html('Xem chi tiáº¿t');
                $('body').removeClass('cart-open');
            }else{
                $('body').addClass('cart-open');
                $(this).parent('.sticky-right').addClass('active');
                $(this).find('.flex-box span').eq(2).html('ÄÃ³ng');
            }
        });
        $('.site-overlay').click(function(){
            $('.cart-mobile').parent('.sticky-right').removeClass('active');
            $('.cart-mobile').find('.flex-box span').eq(2).html('Xem chi tiáº¿t');
            $('body').removeClass('cart-open');
        });
            ...
            $.ajax({
                ...
                success: function(datas){
                    ...
                    $.getJSON('/cart.js', function(data){
                        ...
                        if($('.sticky-right').hasClass('active')){
                            $('.cart-mobile .flex-box').html('<span>'+data.items.length+' MÃ³n</span>' + Haravan.formatMoney(data.total_price, formatMoney) + '</span><span>ÄÃ³ng</span>');
                        }else{
                            $('.cart-mobile .flex-box').html('<span>'+data.items.length+' MÃ³n</span>' + Haravan.formatMoney(data.total_price, formatMoney) + '</span><span>Xem chi tiáº¿t</span>');
                        }
                    });
                }
            });
        ...
        function update_cart(id,quantity){
            ...
            $.ajax({
                ...
                success: function(datas){
                    ...
                    if($('.sticky-right').hasClass('active')){
                        $('.cart-mobile .flex-box').html('<span>'+datas.items.length+' MÃ³n</span>' + Haravan.formatMoney(datas.total_price, formatMoney) + '</span><span>ÄÃ³ng</span>');
                    }
                    else{
                        $('.cart-mobile .flex-box').html('<span>'+datas.items.length+' MÃ³n</span>' + Haravan.formatMoney(datas.total_price, formatMoney) + '</span><span>Xem chi tiáº¿t</span>');
                    }
        ...
        $('.sticky-right').click(function(e){
            ...
            if (!$(target).closest('.cart-mobile').length && !$(target).closest('.orders-cart').length ) {
                ...
                $('body').removeClass('cart-open');
                $('.cart-mobile .flex-box span').eq(2).html('Xem chi tiáº¿t');
            }
        });
        ...
        var slug = function (e) {
            return (e = (e = (e = (e = (e = (e = (e = (e = (e = (e = (e = e.toLowerCase()).replace(/Ã |Ã¡|áº¡|áº£|Ã£|Ã¢|áº§|áº¥|áº­|áº©|áº«|Äƒ|áº±|áº¯|áº·|áº³|áºµ/g, "a")).replace(/Ã¨|Ã©|áº¹|áº»|áº½|Ãª|á»|áº¿|á»‡|á»ƒ|á»…/g, "e")).replace(/Ã¬|Ã­|á»‹|á»‰|Ä©/g, "i")).replace(
                /Ã²|Ã³|á»|á»|Ãµ|Ã´|á»“|á»‘|á»™|á»•|á»—|Æ¡|á»|á»›|á»£|á»Ÿ|á»¡/g,
                "o"
            )).replace(/Ã¹|Ãº|á»¥|á»§|Å©|Æ°|á»«|á»©|á»±|á»­|á»¯/g, "u")).replace(/á»³|Ã½|á»µ|á»·|á»¹/g, "y")).replace(/Ä‘/g, "d")).replace(/!|@|%|\^|\*|\(|\)|\+|\=|\<|\>|\?|\/|,|\.|\:|\;|\'| |\"|\&|\#|\[|\]|~|$|_/g, "-")).replace(/-+-/g, "-")).replace(/^\-+|\-+$/g, ""));
        }
...
CR.Index = {
    ...
    loadMoreStyle1: function(){
        ...
        jQuery(document).on("click",".home-tablist--col.style-1 .viewall-collection button", function(e){
            ...
            var total_num = $(this).attr('data-total'); // Sá»‘ lÆ°á»£ng sáº£n pháº©m cá»§a nhÃ³m
